...
function HeroSection() {
  const { featuredProducts, loading } = useHomeData();
  ...
  const safeCurrentIndex =
    products.length > 0
      ? Math.min(currentIndex, products.length - 1)
      : 0;
  ...
  const currentProduct = products[safeCurrentIndex] || null;
  ...
  const getImageUrl = (image) => {
    ...
    if (image.startsWith("http")) {
      return `${image}?tr=w-650,q-82`;
    }

    return `https://ecommerce-platform-4vwn.onrender.com/storage/${image}`;
  };

  const imageUrl = primaryImage?.image
    ? getImageUrl(primaryImage.image)
    : null;
      ...
      <div className="nova-hero-container">
        <div className="nova-hero-content">
          ...
          <p className="nova-hero-description">
            Explore trending technology, fashion,
            beauty, home essentials and more in one
